...
function getThemeDotColors() {
  ...
  return {
    background: normalizeHex(root.getPropertyValue("--bg-color")) ?? "#0a0a0a",
    base: normalizeHex(root.getPropertyValue("--secondary-color")) ?? "#404040",
    glow: normalizeHex(root.getPropertyValue("--primary-color")) ?? "#f8c915",
  }
}
...
export function DotPattern({
  ...
  const [themeColors, setThemeColors] = useState(() => getThemeDotColors())

  const resolvedBackground = baseColor ? undefined : themeColors.background
  const resolvedBaseColor = baseColor ?? themeColors.base
  const resolvedGlowColor = glowColor ?? themeColors.glow

  const baseRgb = useMemo(() => hexToRgb(resolvedBaseColor), [resolvedBaseColor])
  const glowRgb = useMemo(() => hexToRgb(resolvedGlowColor), [resolvedGlowColor])

  useEffect(() => {
    const root = document.documentElement
    const updateThemeColors = () => setThemeColors(getThemeDotColors())
  ...
  return (
    ...
    >
      ...
      {/* Vignette overlay - Fixed position */}
      <div
        className="fixed inset-0 pointer-events-none"
        style={{
          background: resolvedBackground
            ? `radial-gradient(ellipse at center, transparent 0%, transparent 40%, ${resolvedBackground}cc 100%)`
            : "radial-gradient(ellipse at center, transparent 0%, transparent 40%, hsl(var(--background) / 0.8) 100%)",
          zIndex: 1,
        }}
      />

      {/* Content layer - Relative position for scrolling */}
      {children && (
